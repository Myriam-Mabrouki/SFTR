/********
* ec2c version 0.67
* c main file generated for node : JAFTER 
...
/* Output procedures **********************/
#ifdef CKCHECK
void JAFTER_BOT_y(void* cdata){
   _put_bottom("y");
}
#endif
/* Output procedures **********************/
void JAFTER_O_y(void* cdata, _boolean _V) {
   _put_bool("y", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct JAFTER_ctx* ctx = JAFTER_new_ctx(NULL);
   JAFTER_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
