/********
* ec2c version 0.67
* c main file generated for node : EDGE2 
...
/* Output procedures **********************/
#ifdef CKCHECK
void EDGE2_BOT_y(void* cdata){
   _put_bottom("y");
}
#endif
/* Output procedures **********************/
void EDGE2_O_y(void* cdata, _boolean _V) {
   _put_bool("y", _V);
}/* Main procedure *************************/
int main(){
   ...
   int s = 0;
   /* Context allocation */
   struct EDGE2_ctx* ctx = EDGE2_new_ctx(NULL);
   EDGE2_reset(ctx);
   /* Main loop */
   ISATTY = isatty(0);
   while(1){
